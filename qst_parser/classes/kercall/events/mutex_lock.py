"""
QST Parser - Mutex Lock 事件解析器

解析 __KER_SYNC_MUTEX_LOCK 内核调用事件。
...
from ..models import MutexLockEnterEvent, MutexLockExitEvent
from ..constants import KernelCall


class MutexLockEventParser:
    """Mutex Lock 事件解析器"""
    
    KERCALL_NUM = KernelCall.SYNC_MUTEX_LOCK
    KERCALL_NAME = "SYNC_MUTEX_LOCK"
    ...
    def parse_enter(
        ...
        timestamp_ns: int,
        is_64bit: bool = False
    ) -> Optional[MutexLockEnterEvent]:
        """
        解析 __KER_SYNC_MUTEX_LOCK ENTER 事件
        ...
        Returns:
            MutexLockEnterEvent 或 None（组合事件未完成时）
        """
    ...
    def _parse_enter_wide(
        ...
        key = f"mutex_lock_enter_{raw.cpu_id}_{raw.data[0]}"
        ...
        if struct_type == StructType.COMBINE_BEGIN:
            ...
            matching_key = self._find_key(raw, self._enter_buffers, "mutex_lock_enter_")
            if matching_key:
    ...
    def parse_exit(
        ...
        timestamp_ns: int,
        is_64bit: bool = False
    ) -> Optional[MutexLockExitEvent]:
        """
        解析 __KER_SYNC_MUTEX_LOCK EXIT 事件
        ...
        # Wide mode 组合事件 (EXIT 通常不需要，但保留处理)
    ...
    def _parse_exit_wide(
        ...
        key = f"mutex_lock_exit_{raw.cpu_id}_{raw.data[0]}"
        ...
        if struct_type == StructType.COMBINE_BEGIN:
            ...
            matching_key = self._find_key(raw, self._exit_buffers, "mutex_lock_exit_")
            if matching_key:
                self._exit_buffers[matching_key]['wide_data'].extend([raw.data[1], raw.data[2]])
            return None
