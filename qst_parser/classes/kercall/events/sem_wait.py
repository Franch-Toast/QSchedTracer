"""
QST Parser - Semaphore Wait 事件解析器

解析 __KER_SYNC_SEM_WAIT 内核调用事件。
...
ENTER (64-bit, _NTO_TRACE_KERCALL64):
    ...
    64-bit Wide 模式数据布局:
        ...
        COMBINE_CONT:  timestamp, try, count (如果需要)
        COMBINE_END:   timestamp, ..., owner
...
from ..models import SemWaitEnterEvent, SemWaitExitEvent
from ..constants import KernelCall


class SemWaitEventParser:
    """Semaphore Wait 事件解析器"""
    
    KERCALL_NUM = KernelCall.SYNC_SEM_WAIT
    KERCALL_NAME = "SYNC_SEM_WAIT"
    ...
    def parse_enter(
        ...
        timestamp_ns: int,
        is_64bit: bool = False
    ) -> Optional[SemWaitEnterEvent]:
        """
        解析 __KER_SYNC_SEM_WAIT ENTER 事件
        """
    ...
    def _parse_enter_wide(
        ...
        key = f"sem_wait_enter_{raw.cpu_id}_{raw.data[0]}"
        ...
        if struct_type == StructType.COMBINE_BEGIN:
            if is_64bit:
                ...
                # 32-bit Wide BEGIN: timestamp, sync_p, try
                self._enter_buffers[key] = {
                    'timestamp_cycles': raw.data[0],
                    'timestamp_ns': timestamp_ns,
                    'cpu_id': raw.cpu_id,
            ...
            matching_key = self._find_key(raw, self._enter_buffers, "sem_wait_enter_")
            if matching_key:
            ...
            if matching_key:
                ...
                if buf['is_64bit']:
                    # 64-bit Wide: sync_p (64), try, count, owner
                    # COMBINE_END 数据: try, count, owner
                    try_flag = wide_data[0] if len(wide_data) > 0 else 0
                    count = wide_data[1] if len(wide_data) > 1 else 0
                    owner = wide_data[2] if len(wide_data) > 2 else 0
                    event = SemWaitEnterEvent(
                    ...
                    # 32-bit Wide: sync_p, try, count, owner
                    # COMBINE_END 数据: count, owner
                    ...
                    event = SemWaitEnterEvent(
                        ...
                        # is_wide=True,
                        # data1=buf['sync_ptr'],
                        # data2=buf.get('try_flag', 0),
                        sync_ptr=buf['sync_ptr'],
                        try_flag=buf.get('try_flag', 0),
                        count=count,
                        owner=owner,
                    )
    ...
    def parse_exit(
        ...
        timestamp_ns: int,
        is_64bit: bool = False
    ) -> Optional[SemWaitExitEvent]:
        """解析 __KER_SYNC_SEM_WAIT EXIT 事件"""
        struct_type = raw.struct_type
    ...
    def _parse_exit_wide(
        ...
        key = f"sem_wait_exit_{raw.cpu_id}_{raw.data[0]}"
        ...
        if struct_type == StructType.COMBINE_BEGIN:
            ...
            matching_key = self._find_key(raw, self._exit_buffers, "sem_wait_exit_")
            if matching_key:
                buf = self._exit_buffers[matching_key]
                
                event = SemWaitExitEvent(
                    timestamp_cycles=buf['timestamp_cycles'],
                    timestamp_ns=buf['timestamp_ns'],
                    cpu_id=buf['cpu_id'],
                    kercall_num=self.KERCALL_NUM,
